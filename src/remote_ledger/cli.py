...
def cmd_validate(args: argparse.Namespace) -> int:
    root = _repo_root()
    targets = (
        [Path(args.path)] if args.path and Path(args.path).is_file()
        else corpus_files(Path(args.path) if args.path else root)
    )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="rl", description="Remote Ledger: compile and check IR remote files."
    )
    ...
    v = sub.add_parser("validate", help="schema and semantic checks (R15)")
    ...
    b = sub.add_parser("build", help="the whole pipeline, whole-tree (D19)")
    b.add_argument("--check", action="store_true", help="diff instead of write")
    b.set_defaults(func=cmd_build)

    for name, phase, help_text in (
        ("check", 2, "cross-check every candidate group (R13)"),
        ("compile", 2, "write build/pronto/... (R12)"),
        ...
        sp = sub.add_parser(name, help=f"{help_text} [phase {phase}]")
